...
{formatted_messages}"""
    ...
    try:
        # Use reasoner (thinking mode) for deep analysis, chat for regular summaries
        model = "deepseek-reasoner" if deep else "deepseek-chat"
        max_tokens = 2000 if deep else 1000
        
        response = deepseek_client.chat.completions.create(
            model=model,  # V3.2 - both models same price now
            messages=[
                {
                    "role": "system",
...
def parse_user_request(content: str):
    ...
    # Message count
    count_match = re.search(r'(\d+)\s*(messages?|msgs?)?(?!\s*h)', content_lower)
    if count_match:
    ...
    # Common time phrases
    if 'last hour' in content_lower:
        result['hours'] = 1
    elif 'last day' in content_lower or 'past day' in content_lower or '24 hours' in content_lower:
        result['hours'] = 24
    elif 'last week' in content_lower or 'past week' in content_lower:
        result['hours'] = 168
